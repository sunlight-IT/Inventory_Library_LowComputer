...
void UartAction_ServoMotor(uint8_t *buf, uint8_t len)
{
	...
	{
		switch (ServoMotor.UartCurrentCmd[1])
		{ // 寄存器
		case MonitorRegister_1_Data:
		{ // 含电机脉冲、扭矩、速度等
		}
		break;
		case MonitorRegister_1_Set:
		{ // 状态寄存器1设置地址，需要设置该位读取“回授位置（PUU）自定义单位，00h”
		...
		break;
		case GearRatioRegister_Numerator:
		{ // 电子齿轮比分子分母寄存器，需要设置分子160，分母94
			switch (ServoMotor.UartCurrentCmd[2])
			{
			case 4:
			{
		...
		{ // 0:关闭所有下述功能;1：强制软件 Servo On。5：设定可防止连续写入EEPROM,若使用通讯控制时必需将此参数设定。
			switch (ServoMotor.UartCurrentCmd[2])
			{
			case 1:
			{
				if (uartbuf[4] == (EEPROM_Disable_P2_30 & 0xFF))
				{ // 已经设置了EEPROM不保存
		...
		{ // 脉冲清除模式，需要设置为0x11;
			...
			case 1:
			{
				if (uartbuf[4] == (PulseReset_Data_P2_50 & 0xFF))
				{ // 已经设置脉冲清除模式，为0x11;
	...
	case Cmd_Write_Once_Func_ServoMotor:
	{
		...
		case JOGmode_Register_P4_05:
		{
			...
			case 9:
			{
				if (ServoMotor.MotorPosition == Right_Position_ServoMotor)
				{ // 右//CCW往左动JOGmode_Data_CCW
					ServoMotor.Flag_Event[2] = JOGmode_Data_CCW == (((uint16_t)uartbuf[4] << 8) | uartbuf[5]) ? 10 : 8;
				}
				else if (ServoMotor.MotorPosition == Left_Position_ServoMotor)
				{ // 左//CW往右动
	...
	case Cmd_Write_Multi_Func_ServoMotor:
	{
		...
		{ // 电子齿轮比分子分母寄存器，需要设置分子160，分母94
			...
			case 4:
			{
				if (uartbuf[5] == ServoMotor.UartCurrentCmd[2])
					ServoMotor.Flag_Event[1] = 14; // 重新电子齿轮比分子分母寄存器
			}
			break;
			}
		}
...
void Event_Movement_ServoMotor(void)
{
	...
	break;
	case 2:
	{ // 设备最低速度缓慢运动然后当刚离开传感器范围，随后又返回至刚触发瞬间
	...
	case 10:
	{
		if (LimitSensorOFF_ServoMotor == HAL_GPIO_ReadPin(LimitSensor_ServoMotor_GPIO_Port, LimitSensor_ServoMotor_Pin))
		{ // 刚好离开，马上停止
			ResetTimerFlag_ServoMotor(&ServoMotor.TimerFlag[1]);
	...
	break;
	case 20:
	{ // 设置了PT模式，发送清除脉冲的指令，复位DI2
		ResetTimerFlag_ServoMotor(&ServoMotor.TimerFlag[1]);
		if (0x00 == GetCmdType())
		{
			Uart_Send_MovementRegister_ServoMotor(3, 0); // 设置速度1，
...
void Uart_Send_MovementRegister_ServoMotor(uint8_t mode, uint16_t Data)
{
	...
	break;
	case 2:
	{ // 写入PT或Sz模式,写入4，设置速度模式，写入0，设置位置模式
	...
	break;
	case 4:
	{ // DI2设置0x0004-脉冲清除,0x0104	//脉冲恢复计数
...
void UartSend_MonitorRegister_ServoMotor(uint8_t mode)
{
	...
	break;
	case 7:
	{ // 写入正确的电子齿轮比分子分母的设置，分子160，分母94，对应的皮带94mm一圈，计算得出1000脉冲/mm
	...
	break;
	case 10:
	{ // 读一次EEPROM的设置，0:关闭所有下述功能;1：强制软件 Servo On。5：设定可防止连续写入EEPROM,若使用通讯控制时必需将此参数设定。
		Uart_ReadWriteRegister_ServoMotor(Cmd_Read_Func_ServoMotor, 1, AuxrFuncRegister_P2_30, 0, Set_null_ServoMotor);
	}
	break;
	case 11:
	{ // 设置EEPROM的为不保存模式，0:关闭所有下述功能;1：强制软件 Servo On。5：设定可防止连续写入EEPROM,若使用通讯控制时必需将此参数设定。
...
{
	return ServoMotor.IsPowerOn;
}

void UartReceiveIT_ServoMotor(void)
{																			 // 读写器串口接收
	HAL_UART_Receive_IT(&huart_ServoMotor, (uint8_t *)Rxdbuf_ServoMotor, 1); // 重新启动接收中断
}
void UartGetData_ServoMotor(void)
{ // 处理接收到的数据到新的数组，并重新接收
...
void UartRegroup_ASCII_ServoMotor(void)
{
	...
	case 0:
	{ // 帧头必须是':'(0x3A)
		if (Rxdbuf_ServoMotor[0] == ':')
			Flag_UartRegroup_ASCII_ServoMotor = 1;
	}
	break;
	case 1:
	{ // 高字节
...
void UartWrite_ServoMotor(uint8_t *pData, uint16_t Size)
{ // 发送给读写器
